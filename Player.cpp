...
void Player::update(){//TODO: ugly, it still does not work ideally, but it works with negative positions
	...
	int mX = floorf(x / tileSize);
	int mY = floorf(y / tileSize);
	...
	if(key_states[SDL_SCANCODE_D]){
		if(!map->isSolid(floorf((x + PLAYER_SPEED) / tileSize) + 1, mY)
			&& !(!onGround && map->isSolid(floorf((x + PLAYER_SPEED) / tileSize) + 1, mY + 1))
			){
			x += PLAYER_SPEED;
		...
		if(!map->isSolid(floorf((x - PLAYER_SPEED) / tileSize), mY)
			&& !(!onGround && map->isSolid(floorf((x - PLAYER_SPEED) / tileSize), mY + 1))
			){
			x -= PLAYER_SPEED;
		} else{
	...
	mX = floorf(x / tileSize);

	float distanceToTileBoundary = (mY - y / tileSize)*tileSize;
	if(!map->isSolid(mX, floorf((y + yVel) / tileSize))
		&& !(mX - (x / tileSize) != 0.f && map->isSolid(mX + 1, floorf((y + yVel) / tileSize)))){
		y += yVel;
	} else{
		yVel = distanceToTileBoundary;
	}

	mY = floorf(y / tileSize);
	onGround = map->isSolid(mX, mY + 1) || (mX - (x / tileSize) != 0.f && map->isSolid(mX + 1, mY + 1));
