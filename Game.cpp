...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char** argv){
	...
	if(tilemap==nullptr){
		SDL_LogWarn(SDL_LOG_CATEGORY_ERROR, "Couldn't load the tilemap: %s", SDL_GetError());
		return SDL_APP_FAILURE;
	}
	...
	for(size_t i = 0; i < tileMapHeight; i++){
		for(size_t j = 0; j < tileMapWidth; j++){
			...
			if(!SDL_BlitSurface(tilemap, &srcRect, temp, &tileRect)){
				SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Couldn't process the tilemap: %s", SDL_GetError());
				return SDL_APP_FAILURE;
			}
			tiles[i * tileMapWidth + j] = SDL_CreateTextureFromSurface(renderer,temp);
			if(tiles[i * tileMapWidth + j]==nullptr){
				SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Couldn't upload tiles to GPU: %s", SDL_GetError());
				return SDL_APP_FAILURE;
			}
		}
	}
